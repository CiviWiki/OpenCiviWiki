...
class Representative(models.Model):
    ...
    def summarize(self, account=None):
        ...
        data = {
            ...
            "party": self.party,
            "state": "{state}".format(
                state=dict(US_STATES).get(self.state, self.state)
            ),
            "profile_image": "https://theunitedstates.io/images/congress/225x275/{}.jpg".format(
                self.bioguide_id
            ),
        }
        if account:
            ...
            votes["compatibility_score"] = (
                voted_same_count / voted_total if voted_total else 0
            )
