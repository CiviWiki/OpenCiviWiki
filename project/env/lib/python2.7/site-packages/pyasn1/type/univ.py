    ...
    if sys.version_info[0] <= 2:
        def __div__(self, value):
            if isinstance(value, float):
                return Real(old_div(self._value, value))
            else:
                return self.clone(old_div(self._value, value))

        def __rdiv__(self, value):
            if isinstance(value, float):
                return Real(old_div(value, self._value))
            else:
                return self.clone(old_div(value, self._value))
    else:
...
class Real(base.AbstractSimpleAsn1Item):
    ...
    if sys.version_info[0] <= 2:
        def __div__(self, value):
            return self.clone(old_div(float(self), value))

        def __rdiv__(self, value):
            return self.clone(old_div(value, float(self)))
    else:
        def __truediv__(self, value):
            return self.clone(old_div(float(self), value))
