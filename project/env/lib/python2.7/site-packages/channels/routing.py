    ...
    def resolve_routing(cls, routing):
        ...
        if isinstance(routing, dict):
            routing = [
                ...
                for channel, consumer in list(routing.items())
            ]
        return routing
...
class Route(object):
    ...
    def __init__(self, channels, consumer, **kwargs):
        ...
        for name, regex in list(self.filters.items()):
    ...
    def match(self, message):
        ...
        call_args = {}
        for name, value in list(self.filters.items()):
    ...
    def __str__(self):
        return "%s %s -> %s" % (
            ...
            "" if not self.filters else "(%s)" % (
                ", ".join("%s=%s" % (n, v.pattern) for n, v in list(self.filters.items()))
            ),
...
class Include(object):
    ...
    def match(self, message):
        ...
        call_args = {}
        for name, prefix in list(self.prefixes.items()):
            if name not in message:
                return None
