...
COMPRESSION_INFO_REV = dict([(v, k) for (k, v) in list(COMPRESSION_INFO.items())])
...
class ImageFileDirectory_v2(collections.MutableMapping):
    ...
    def named(self):
        ...
        return dict((TiffTags.lookup(code).name, value)
                    for code, value in list(self.items()))
...
def _save(im, fp, filename):
    ...
    if libtiff:
        ...
        for tag, value in itertools.chain(list(ifd.items()),
                                          list(getattr(im, 'tag_v2', {}).items()),
                                          list(legacy_ifd.items())):
            ...
            if tag not in atts and tag not in blocklist:
                if isinstance(value, str if bytes is str else str):
...
class AppendingTiffWriter(object):
    fieldSizes = [
        0,  # None
        1,  # byte
        1,  # ascii
