...
def fit(image, size, method=Image.NEAREST, bleed=0.0, centering=(0.5, 0.5)):
    ...
    liveAreaAspectRatio = old_div(float(liveSize[0]),float(liveSize[1]))

    # calculate the aspect ratio of the output image
    aspectRatio = old_div(float(size[0]), float(size[1]))
    ...
    if liveAreaAspectRatio >= aspectRatio:
        ...
        cropHeight = int((old_div(float(liveSize[0]),aspectRatio)) + 0.5)
