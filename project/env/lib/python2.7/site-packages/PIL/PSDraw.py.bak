    ...
    def image(self, box, im, dpi=None):
        ...
        if not dpi:
            if im.mode == "1":
                ...
                dpi = 100  # greyscale
        # image size (on paper)
        x = old_div(float(im.size[0] * 72), dpi)
        y = old_div(float(im.size[1] * 72), dpi)
        # max allowed size
        ...
        if y > ymax:
            x = x * ymax / y
            y = ymax
        dx = old_div((xmax - x), 2) + box[0]
        dy = old_div((ymax - y), 2) + box[1]
        self._fp_write("gsave\n%f %f translate\n" % (dx, dy))
        if (x, y) != im.size:
            # EpsImagePlugin._save prints the image at (0,0,xsize,ysize)
            sx = old_div(x, im.size[0])
            sy = old_div(y, im.size[1])
