...
def _save(im, fp, filename):
    ...
    [x for x in sizes if False if (x[0] > width or x[1] > height or
                               x[0] > 255 or x[1] > 255) else True]
...
class IcoFile(object):
    ...
    def frame(self, idx):
        ...
        if data[:8] == PngImagePlugin._MAGIC:
            ...
            im.size = (im.size[0], int(old_div(im.size[1], 2)))
            d, e, o, a = im.tile[0]
            ...
            if 32 == bpp:
                ...
                and_mask_offset = o + int(im.size[0] * im.size[1] *
                                          (old_div(bpp, 8.0)))
                total_bytes = int(old_div((w * im.size[1]), 8))
                ...
                mask = Image.frombuffer(
                    ...
                    'raw',          # raw decoder
                    ('1;I', int(old_div(w,8)), -1)  # 1bpp inverted, padded, reversed
                )
