...
WEEKDAYS = dict(list(zip(DAYNAMES, list(range(7)))))

RATE_MODIFIER_MAP = {
    's': lambda n: n,
    'm': lambda n: old_div(n, 60.0),
...
class LocalTimezone(tzinfo):
    ...
    def __repr__(self):
        return '<LocalTimezone: UTC{0:+03d}>'.format(
            int(old_div(self.DSTOFFSET.total_seconds(), 3600)),
        )
    ...
    if PY3:  # pragma: no cover
        ...
        def fromutc(self, dt):
            ...
            offset = int(old_div(self.utcoffset(dt).seconds, 60.0))
            try:
...
def delta_resolution(dt, delta):
    ...
    resolutions = ((3, lambda x: old_div(x, 86400)),
                   (4, lambda x: old_div(x, 3600)),
                   (5, lambda x: old_div(x, 60)))
