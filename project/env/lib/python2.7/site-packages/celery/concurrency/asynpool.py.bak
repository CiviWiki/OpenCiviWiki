...
SCHED_STRATEGY_TO_NAME = {v: k for k, v in list(SCHED_STRATEGIES.items())}
...
class AsynPool(_pool.Pool):
    ...
    def human_write_stats(self):
        ...
        def per(v, total):
            return '{0:.2%}'.format((old_div(float(v), total)) if v else 0)

        return {
            'total': total,
            'avg': per(old_div(total, len(self.write_stats)) if total else 0, total),
