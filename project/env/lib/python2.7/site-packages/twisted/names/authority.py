...
class PySourceAuthority(FileAuthority):
    ...
    def setupConfigNamespace(self):
        r = {}
        items = iter(list(dns.__dict__.keys()))
...
class BindAuthority(FileAuthority):
        ...
        for line in lines:
            if state == 0:
                ...
                if line.find(b')') != -1:
                    ...
                    l[-1] += b' ' + line
        return [_f for _f in (line.split() for line in l) if _f]
    ...
    def parseRecordLine(self, origin, ttl, line):
        ...
        if _PY3:
            queryClasses = set(
                qc.encode("ascii") for qc in list(dns.QUERY_CLASSES.values())
            )
            queryTypes = set(
                qt.encode("ascii") for qt in list(dns.QUERY_TYPES.values())
            )
        else:
