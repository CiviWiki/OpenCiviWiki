...
try:
    ...
    def closerange(fd_low, fd_high):  # noqa
        for fd in reversed(list(range(fd_low, fd_high))):
            try:
                os.close(fd)
    ...
    def close_open_fds(keep=None):  # noqa
        ...
        for fd in reversed(list(range(get_fdmax(default=2048)))):
            if fd not in keep:
                try:
                    os.close(fd)
                except OSError as exc:
...
if sys.platform == 'win32':
    ...
    def mem_rss():
        ...
        return int(old_div(psutil.Process(os.getpid()).memory_info()[0], 1024.0))
