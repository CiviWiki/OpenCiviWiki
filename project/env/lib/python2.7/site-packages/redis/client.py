...
from redis._compat import (b, basestring, bytes, imap, iteritems, iterkeys,
                           itervalues, izip, int, nativestr, str,
                           safe_unicode)
...
def parse_sentinel_master(response):
    return parse_sentinel_state(list(map(nativestr, response)))


def parse_sentinel_masters(response):
    result = {}
    for item in response:
        state = parse_sentinel_state(list(map(nativestr, item)))
        result[state['name']] = state
    return result


def parse_sentinel_slaves_and_sentinels(response):
    return [parse_sentinel_state(list(map(nativestr, item))) for item in response]
...
class StrictRedis(object):
    ...
    def pexpire(self, name, time):
        ...
        """
        if isinstance(time, datetime.timedelta):
            ms = int(old_div(time.microseconds, 1000))
    ...
    def psetex(self, name, time_ms, value):
        ...
        if isinstance(time_ms, datetime.timedelta):
            ms = int(old_div(time_ms.microseconds, 1000))
    ...
    def set(self, name, value, ex=None, px=None, nx=False, xx=False):
        ...
        if px:
            ...
            if isinstance(px, datetime.timedelta):
                ms = int(old_div(px.microseconds, 1000))
...
class PubSub(object):
    ...
    def psubscribe(self, *args, **kwargs):
        ...
        new_patterns = {}
        new_patterns.update(dict.fromkeys(list(map(self.encode, args))))
    ...
    def subscribe(self, *args, **kwargs):
        ...
        if args:
            args = list_or_args(args[0], args[1:])
        new_channels = {}
        new_channels.update(dict.fromkeys(list(map(self.encode, args))))
...
class BasePipeline(object):
    ...
    def annotate_exception(self, exception, number, command):
        cmd = safe_unicode(' ').join(map(safe_unicode, command))
