...
cw.AccountView = BB.View.extend({
  ...
  initialize: function(options) {
    ...
    this.listenTo(this.model, "sync", function() {
      ...
      if (
        _.find(this.model.get("followers"), function(follower) {
  ...
  render: function() {
    if (this.isSave) {
      ...
      $(".account-tabs .tab").on("dragstart", function() {
        return false;
      });
      this.$el
        .find(".account-settings")
        .pushpin({ top: $(".account-settings").offset().top });
  ...
  tabsRender: function() {
    this.$("#civis")
      .empty()
      .append(this.mycivisTemplate());
    this.$("#followers")
      .empty()
      .append(this.followersTemplate());
    this.$("#following")
      .empty()
      .append(this.followingTemplate());
    this.$("#issues")
      .empty()
      .append(this.myissuesTemplate());
    this.$("#myreps")
      .empty()
      .append(this.myrepsTemplate());
    this.$("#mybills")
      .empty()
  ...
  postRender: function() {
    ...
    this.model.set({
      profile_image:
        this.model.get("profile_image") + "?" + new Date().getTime()
    });
    this.$el
      .find(".account-settings")
      .empty()
      .append(this.sidebarTemplate());
    this.tabsRender();
    cw.materializeShit();
    this.isSave = false;
  },
  ...
  toggleSolutions: function(e) {
    ...
    var new_text =
      textElement.text() === "Show Solutions"
        ? "Hide Solutions"
        : "Show Solutions";
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            _this
              .$(e.currentTarget)
              .parent()
              .after(_this.userCardTemplate(data));
            // Hover Elements
  ...
  followRequest: function(e) {
    ...
    if (!follow_state) {
      $.ajax({
        ...
        success: function() {
          Materialize.toast(
            "You are now following user " + apiData.target,
            5000
          );
  ...
  saveLocation: function() {
    ...
    if (!_.isEmpty(coordinates) && !_.isEmpty(address)) {
      $.ajax({
        ...
        success: function(data) {
          Materialize.toast(
            '<span class="subtitle-lato white-text">Location Changed</span>',
            5000
          );
          _this.isSave = true;
          _this.model.fetch();
