...
cw.CiviModel = BB.Model.extend({
  ...
  url: function() {
    if (!this.id) {
      throw new Error(
        "This is a race condition! and why we can't have nice things :("
      );
    }
  ...
  save: function(attrs, options) {
    // TODO: Remove this after implementing django REST framework
    options = options || {};
...
cw.CiviView = BB.View.extend({
  ...
  previewImageNames: function(e) {
    ...
    _.each(
      ...
      function(img_file) {
        $previewlist.append(
          '<div class="link-lato gray-text preview-item ">' +
            img_file.name +
            "</div>"
        );
      },
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if (!prev_vote) {
            ...
            prev_votes = _.reject(prev_votes, function(v) {
              return v.civi_id === civi_id;
            });
            prev_votes.push(response.data);
            _this.parentView.model.set("user_votes", prev_votes);
          }
  ...
  clickEdit: function(e) {
    ...
    this.$("#" + this.model.get("type") + "-" + this.model.id).prop(
      "checked",
      true
    );
    if (
      this.model.get("type") != "response" &&
      this.model.get("type") != "rebuttal"
    ) {
  ...
  saveEdit: function(e) {
    ...
    var new_body = this.$(".edit-civi-body")
      .val()
      .trim();
              ...
              success: function(response2) {
                ...
                if (
                  ...
                  _.each(
                    parent_links,
                    function(parent_id) {
                      var parent_civi = _this.options.parentView.civis.get(
                        parent_id
                      );
                      if (parent_civi) {
    ...
    if (title && body && c_type) {
      if (links.length === 0) {
        if (c_type === "cause") {
          ...
          Materialize.toast(
            "A SOLUTION Civi must be linked to a CAUSE Civi",
            5000
          );
          this.$(e.currentTarget)
            .removeClass("disabled")
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          var can_edit =
            new_civi.get("author").username == _this.options.parentView.username
              ? true
              : false;
        ...
        success: function(response) {
          ...
          if (_this.attachmentCount > 0) {
            var formData = new FormData(
              _this.$("#response_attachment_image_form")[0]
            );
            formData.set("civi_id", response.data.id);
            if (_this.attachment_links.length) {
              _.each(_this.attachment_links, function(img_link) {
            ...
            $.ajax({
              ...
              error: function(e) {
                Materialize.toast(
                  "Response was created but images could not be uploaded",
                  5000
                );
  ...
  render: function() {
    ...
    this.ms = this.$el.magicSuggest({
      ...
      selectionRenderer: function(data) {
        return (
          '<span class="gray-text bold-text">' +
...
cw.ThreadView = BB.View.extend({
  ...
  threadNavRender: function() {
    ...
    this.$(".thread-nav")
      .empty()
      .append(this.navTemplate(navRenderData));
    this.$(".main-thread").on("scroll", function(e) {
      _this.processCiviScroll();
    });
  },
  ...
  renderOutline: function() {
    ...
    count.totalRec =
      this.civiRecViewTotals.problem +
      ...
      recCount.solution;
    count.totalOther =
      this.civiOtherViewTotals.problem +
    ...
    var scrollPadding =
      this.$(".main-thread").height() -
      this.civiLocations[this.civiLocations.length - 1].height;
  ...
  renderResponses: function() {
    ...
    _.each(
      ...
      function(civi) {
        var can_edit =
          civi.get("author").username == this.username ? true : false;
        var can_respond =
          this.civis.get(this.responseCollection.civiId).get("author")
            .username == this.username
            ? true
            : false;

        var new_civi_view = new cw.CiviView({
          model: civi,
          can_edit: can_edit,
        ...
        if (civi.get("rebuttal")) {
          ...
          var rebuttal_can_edit =
            rebuttal_model.get("author").username == this.username
              ? true
              : false;
          var rebuttal_view = new cw.CiviView({
    ...
    var scrollPadding =
      this.$(".responses").height() - $lastResponseCivi.height();
  ...
  resizeBodyToWindow: function() {
    ...
    $civiThreadScroll.css({
      height: $windowHeight - $civiThreadScroll.offset().top
    });
    var $civiResponseScroll = this.$(".responses");
    $civiResponseScroll.css({
      height: $windowHeight - $civiResponseScroll.offset().top
    });
  },
  ...
  goToCivi: function(e) {
    ...
    this.$(".main-thread").animate(
      {
        scrollTop:
          _.findWhere(this.civiLocations, {
            id: $link.attr("data-civi-nav-id")
          }).top - 15
      },
      250
    );
  ...
  activateNav: function(id) {
    ...
    var $currentNavCivi = _this.$(
      '[data-civi-nav-id="' + _this.currentNavCivi + '"]'
    );
    ...
    if (!_this.navExpanded) {
      ...
      $($currentNavCivi.closest(".civi-nav-wrapper").siblings()[0]).addClass(
        "current"
      );
    } else {
  ...
  loadMoreCivis: function(e) {
    ...
    var addCount =
      cw.DEFAULTS.viewLimit < remaining ? cw.DEFAULTS.viewLimit : remaining;
    ...
    $.ajax({
      ...
      error: function(response) {
        if (response.status === 403) {
          Materialize.toast(
            "You do not have permission to publish the thread",
            5000
          );
        } else if (response.status === 500) {
          Materialize.toast(
            "Server Error: Thread could not be published",
            5000
          );
          _this
            .$(e.currentTarget)
