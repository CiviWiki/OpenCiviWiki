...
const NotificationView = Bb.View.extend({
...
var render_notifications = function(data) {
  ...
  if (unreadCount === 0) {
    ...
    var notified = false;
    if (
      data.unread_list.length > 5 &&
      !notificationsView.initialNotificationShown
    ) {
      Materialize.toast(
        "You have " + data.unread_list.length + " new notifications",
        5000
      );
    ...
    _.each(
      ...
      function(n) {
        ...
        var old_notification = _.findWhere(
          notificationsView.notifications_data,
          { id: n.id }
        );
